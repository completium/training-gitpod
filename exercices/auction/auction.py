...
class Auction(sp.Contract):
    ...
    @sp.entry_point
    def bid(self):
        sp.verify(sp.now < self.data.deadline, "Trop tard !")
        bids = self.data.bids
        sp.verify(~bids.contains(sp.sender), "Vous avez deja mise")
    ...
    def collectTopBid(self):
        sp.verify(sp.now >= self.data.deadline, "Trop tôt !")
    ...
    def claim(self):
        ...
        sp.verify(sp.now >= self.data.deadline, "Trop tôt !")
        sp.verify(sp.sender != self.data.topBidder, "Vous avez gagné !")
